...
use crate::{RecordingCompletionStatus, RecordingConfig, RecordingHandle, RecordingOutput};
...
impl crate::Recorder for Recorder {
    async fn start(&self, config: RecordingConfig) -> Result<RecordingHandle, String> {
        let display = std::env::var("DISPLAY").map_err(|_| {
            "Cannot start recording: DISPLAY is not set (X11 required).".to_string()
        })?;
        ...
        if width == 0 || height == 0 {
            return Err(format!(
                "Cannot start recording: invalid display dimensions {width}x{height}."
            ));
        }
        ...
        let log_file = std::fs::File::create(&log_path)
            .map_err(|e| format!("Failed to create recording log file: {e}"))?;
        ...
        let mut process = command
            .spawn()
            .map_err(|e| format!("Failed to spawn ffmpeg for recording: {e}"))?;
        ...
        if let Err(e) = wait_for_first_output(&path, &mut process).await {
            ...
            return Err(format!("ffmpeg failed to start recording: {e}{detail}"));
        }
        let _ = std::fs::remove_file(&log_path);
    ...
    async fn stop(&self, handle: RecordingHandle) -> Result<RecordingOutput, String> {
        ...
        let mut completion_status = RecordingCompletionStatus::Complete;
        if let Some(pid) = process.id() {
            let pid = nix::unistd::Pid::from_raw(pid as i32);
            if nix::sys::signal::kill(pid, nix::sys::signal::Signal::SIGINT).is_err() {
                completion_status = RecordingCompletionStatus::Incomplete;
            }
        } else {
            completion_status = RecordingCompletionStatus::Incomplete;
        }
        ...
        match tokio::time::timeout(STOP_TIMEOUT, process.wait()).await {
            Ok(Ok(_)) => {}
            Ok(Err(_)) => completion_status = RecordingCompletionStatus::Incomplete,
            Err(_) => {
        ...
        if size_bytes == 0 {
            let _ = std::fs::remove_file(&path);
            return Err("Recording produced an empty file.".to_string());
        }
...
fn query_display_dimensions() -> Result<(u32, u32), String> {
