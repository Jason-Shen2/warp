...
use warp::tui_export::{
    ...
    BlocklistAIContextModel, BlocklistAIController, BlocklistAIInputModel, CommandExecutionSource,
    ConversationSelection, ExecuteCommandEvent, GetRelevantFilesController, IsSharedSessionCreator,
    LocalTtyTerminalManager, PtyIntent, PtyIntentEvent, ShellLaunchData, TerminalManagerTrait,
    TerminalModel, TerminalSurface, TerminalSurfaceInit, TerminalSurfaceResult,
};
...
use warpui_core::elements::tui::{TuiChildView, TuiColumn, TuiElement, TuiParentElement};
...
use super::preview_shim::{self, PreviewAction};
...
impl PtyIntentEvent for RootEvent {
    fn pty_intent(&self) -> Option<PtyIntent> {
        match self {
            RootEvent::ExecuteCommand(event) => Some(PtyIntent::ExecuteCommand(event.clone())),
        }
    }
}
...
impl RootTuiView {
    ...
    fn run_preview_shell_command(&mut self, ctx: &mut ViewContext<Self>) {
        let Some(session_id) = self.model.lock().block_list().active_block().session_id() else {
            return;
        };
        ctx.emit(RootEvent::ExecuteCommand(ExecuteCommandEvent {
...
impl TuiView for RootTuiView {
    ...
    fn render(&self, _app: &AppContext) -> Box<dyn TuiElement> {
        Box::new(preview_shim::wrap(
            TuiColumn::new().with_child(Box::new(TuiChildView::new(&self.transcript))),
        ))
    }
}
...
impl TypedActionView for RootTuiView {
    ...
    fn handle_action(&mut self, action: &PreviewAction, ctx: &mut ViewContext<Self>) {
        match action {
            PreviewAction::RunShellCommand => self.run_preview_shell_command(ctx),
            PreviewAction::SendAgentPrompt => {
                self.conversation_model.update(ctx, |model, ctx| {
                    model.send_prompt("hello world".to_owned(), ctx)
                });
            }
        }
        ctx.notify();
