...
/// Reported maximum recording duration. Enforcement (auto-stop) is a follow-on;
/// for now this is informational so the agent can plan around it.
const MAX_RECORDING_DURATION: Duration = Duration::from_secs(10 * 60);
/// Reported maximum recording size in bytes.
const MAX_RECORDING_SIZE_BYTES: i64 = 1024 * 1024 * 1024;
...
impl StartRecordingExecutor {
    ...
    pub(super) fn execute(
        ...
        // Reserve the single recording slot up front so a concurrent start can't
        // race past the per-display guard while ffmpeg is spinning up.
        ...
        if let Err(error) = controller.update(ctx, |controller, _| controller.try_begin_start()) {
            return ActionExecution::Sync(AIAgentActionResultType::StartRecording(
                StartRecordingResult::Error(error),
            ));
        }
        ...
        ActionExecution::new_async(
            async move {
                ...
                recorder
                    .start(computer_use::RecordingConfig::default())
                    .await
            },
            |result, ctx| match result {
                ...
                Err(error) => {
                    ...
                    AIAgentActionResultType::StartRecording(StartRecordingResult::Error(error))
                }
            },
        )
    }
