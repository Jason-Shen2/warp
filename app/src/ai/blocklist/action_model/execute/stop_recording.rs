...
use crate::{
    ai::{
        ...
        blocklist::action_model::recording_controller::RecordingController,
...
impl StopRecordingExecutor {
    ...
    pub(super) fn execute(
        ...
        #[cfg(not(target_family = "wasm"))]
        {
            ...
            // Pull the live session out now; an unknown id means the recording
            // already stopped (stop is idempotent).
            let session = RecordingController::handle(ctx)
                .update(ctx, |controller, _| controller.take_session(&recording_id));
            let Some(session) = session else {
                return ActionExecution::<()>::Sync(AIAgentActionResultType::StopRecording(
                    StopRecordingResult::Error(format!(
                        "No active recording with id '{recording_id}'."
                    )),
                ))
                .into();
            };
            ...
            ActionExecution::new_async(
                async move {
                    ...
                    let output = match recorder.stop(handle).await {
                        Ok(output) => output,
                        Err(error) => return StopRecordingResult::Error(error),
                    };
                    ...
                    match upload_result {
                        Ok(upload) => {
                            ...
                            let termination_reason = match completion_status {
                                computer_use::RecordingCompletionStatus::Complete => {
                                    "Stopped by agent".to_string()
                                }
                                computer_use::RecordingCompletionStatus::Incomplete => {
                                    "Recording did not finalize cleanly".to_string()
                                }
                            };
